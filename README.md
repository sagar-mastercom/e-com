...
Authentication in rails 7 using devise and devise-jwt with fast_jsonapi response.
...
```
rails new ecomv1 -–api -–database=mysql
```

Here, I have created a rails 7 application using mysql (Default SQLite).
(Note: If you are using mysql then you have to setup database.yml)

## MySQL Configure in config/database.yml
...
```ruby
#db/migrate/<migration file add_jti_to_users>.rb
def change
...
```ruby
#app/serializers/user.serializer.rb
...
If everything worked correctly, we should see the token logged to the console as well as the server response
...
Because we're not including the authorization token in the header, the response status should be unauthorized (401) and the error will be thrown, resulting in a rejected promise.
